...
<div class="space-y-6">
    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800 mt-2">
        <div class="flex items-center gap-4">
            <a href="{{ route('staff.bills.index') }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Add New Bill</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Create a new utility bill for a room.</p>
            </div>
        </div>
    </div>
    ...
    <!-- Form -->
    <form action="{{ route('staff.bills.store') }}" method="POST" class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
        @csrf
        <div class="space-y-6 p-4 sm:p-6">
            ...
            <div>
                ...
                <input type="number" id="amount" name="amount" step="0.01" min="0" required value="{{ old('amount') }}" placeholder="0.00" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 @error('amount') border-red-500 @enderror">
                @error('amount')
                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" id="billing_month" name="billing_month" required value="{{ old('billing_month') }}" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 @error('billing_month') border-red-500 @enderror">
                @error('billing_month')
                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" id="due_date" name="due_date" required value="{{ old('due_date') }}" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 @error('due_date') border-red-500 @enderror">
                @error('due_date')
                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex gap-3 border-t border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-700 sm:p-6">
            ...
            <a href="{{ route('staff.bills.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-300 px-5 py-2.5 text-sm font-medium text-gray-800 hover:bg-gray-400 focus:ring-4 focus:ring-gray-200 dark:bg-gray-600 dark:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-800">
                Cancel
            </a>
        </div>
    </form>
